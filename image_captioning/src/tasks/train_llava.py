...
def log_image_table(images, predicted, labels, epoch):
    ...
    for img, logits, targ in zip(images.cpu(), predicted.cpu(), labels.cpu()):
        ...
        pred = model.processor.decode(pred_ids, skip_special_tokens=True)
...
def create_training(
    ...
    OUTPUT_DIR = f"{ROOT_DIR}/finetuned_models_debug" if debug else f"{ROOT_DIR}/finetuned_models"
    CONFIG = {
        "epochs": epochs,
    ...
    # Initialize W&B run
    run = wandb.init(
        project=("master-thesis-llava-debug" if debug else "master-thesis-llava"),  # name of project
    ...
    model.processor.tokenizer.padding_side = "right"  # during training, one always uses padding on the right
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="LoRA layers to train, separate by space",
    )
    ...
    lora_layers = ["q_proj", "v_proj"]  # QV layers for Llava
    device = args.device
